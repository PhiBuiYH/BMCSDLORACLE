...
namespace BMCSDL.DAL
{
    class DALXacThuc
    {
        ...
        public DataTable ExecuteGetDataIntoDGV()
        {
            ...
            OracleDataAdapter adapter = new OracleDataAdapter("select HOTEN,NGAYSINH,DIACHI,PHAI,CMND,NGAYCAPCMND,DIENTHOAI,EMAIL from TIN.PASSPORTDATA where PASSPORTDATA.TTXACTHUC='Chưa xác thực'", mydb.getConnection);
            adapter.Fill(table);
        ...
        public int ExecuteUpdateTTXacThuc1(string CMND)
        {
            OracleCommand command = new OracleCommand("update TIN.PASSPORTDATA set TTXACTHUC = :xacthuc,IDUSERXT=:iduser where CMND=:cmnd", mydb.getConnectionNV());
        ...
        public int ExecuteUpdateTTXacThuc2(string CMND)
        {
            OracleCommand command = new OracleCommand("update TIN.PASSPORTDATA set TTXACTHUC = :xacthuc,IDUSERXT=:iduser where CMND=:cmnd", mydb.getConnectionNV());
            command.Parameters.Add("xacthuc", OracleDbType.NVarchar2, 50).Value = "Không xác thực";
            command.Parameters.Add("iduser", OracleDbType.NVarchar2, 50).Value = GlobalVariables.GlobalUserID.ToString();
            command.Parameters.Add("cmnd", OracleDbType.Varchar2, 40).Value = CMND;
